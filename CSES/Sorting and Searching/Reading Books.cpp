...
void solve(){
    ...
    sort(vr(a));
    ll time = 0;
    ll last_maxi_time = a.back();
    ll least_mini_time = accumulate(a.begin(),a.end()-1,0ll);
    if(last_maxi_time <= least_mini_time){
        cout<<last_maxi_time + least_mini_time<<endl;
    }
    else{
        cout<<last_maxi_time*2<<endl;
    }
}
